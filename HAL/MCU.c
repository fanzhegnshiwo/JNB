...
/* 头文件包含 */
#include "HAL.h"
uint8 addrmac_char[6];
extern uint8 scanRspData[];
extern uint8 addrmac_str1[32];
uint8 addrmac[6];
...
void CH58x_BLEInit(void)
{
    ...
    for(i = 0; i < 12; i++)
    {
        ...
        attDeviceName[3+i]=addrmac_str1[i];
    }
...
#else
    {
        ...
        for(i = 0; i < 6; i++)
        {
            ...
            addrmac[5-i] = MacAddr[i];
        }
        hex_to_asciistring(addrmac,12,addrmac_str1);
        for(i = 0; i < 12; i++)
        {
            scanRspData[5+i]=addrmac_str1[i];
        }
